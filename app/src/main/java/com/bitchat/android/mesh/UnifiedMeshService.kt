...
class UnifiedMeshService(
    ...
    override fun stopServices() {
        bluetooth.stopServices()
        try { WifiAwareController.stop() } catch (_: Exception) { }
    }

    override fun sendRawBroadcast(packet: com.bitchat.android.protocol.BitchatPacket) {
        if (isBleEnabled()) {
            bluetooth.sendRawBroadcast(packet)
        }
        // Wifi aware broadcast if needed...
    }
    ...
    override fun sendFileBroadcast(file: BitchatFilePacket) {
        when {
            file.fileSize > 4096 && wifiService() != null -> {
                try {
                    wifiService()?.sendFileBroadcast(file)
                } catch (e: Exception) {
                    if (isBleEnabled()) bluetooth.sendFileBroadcast(file)
                }
            }
    ...
    override fun sendFilePrivate(recipientPeerID: String, file: BitchatFilePacket) {
        when {
            file.fileSize > 4096 && isWifiReady(recipientPeerID) -> {
                try {
                    wifiService()?.sendFilePrivate(recipientPeerID, file)
                } catch (e: Exception) {
                    if (isBleReady(recipientPeerID)) bluetooth.sendFilePrivate(recipientPeerID, file)
                }
            }
