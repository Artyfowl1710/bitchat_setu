...
class MessageHandler(private val myPeerID: String, private val appContext: android.content.Context) {
    ...
    suspend fun handleSosRelay(routed: RoutedPacket) {
        val packet = routed.packet
        val sosPacket = SosRelayPacket.decode(packet.payload) ?: return
        
        Log.d(TAG, "🚨 SOS_RELAY received from ${routed.peerID ?: "unknown"} for ${sosPacket.destinationPhoneNumber}")
        ...
        handlerScope.launch {
            try {
                ...
                if (hasInternet) {
                    ...
                    val url = URL("http://10.88.81.73:3000/relay-sos")
                    ...
                    Log.d(TAG, "SOS Backend response: $responseCode")
                    ...
                    if (responseCode in 200..299) {
                        ...
                        delegate?.sendPacket(deliveredPacket)
                    }
                }
            } catch (e: Exception) {
                Log.e(TAG, "Failed to process SOS_RELAY: ${e.message}")
            }
        }
    }

    suspend fun handleSosDelivered(routed: RoutedPacket) {
        val messageId = String(routed.packet.payload, Charsets.UTF_8)
        Log.d(TAG, "SOS Delivered confirmed by mesh: $messageId")
        delegate?.onDeliveryAckReceived(messageId, routed.peerID ?: "unknown")
    }
