...
class BluetoothMeshService(private val context: Context) : TransportBridgeService.TransportLayer {
    ...
    fun sendRawBroadcast(packet: com.bitchat.android.protocol.BitchatPacket) {
        if (!isBleTransportEnabled()) return
        val routed = RoutedPacket(packet, null)
        connectionManager.broadcastPacket(routed)
        Log.d(TAG, "sendRawBroadcast: type=${packet.type}")
    }
    ...
    private fun setupDelegates() {
        ...
        packetProcessor.delegate = object : PacketProcessorDelegate {
            ...
            override fun handleSosRelay(routed: RoutedPacket) {
                serviceScope.launch { messageHandler.handleSosRelay(routed) }
            }

            override fun handleSosDelivered(routed: RoutedPacket) {
                serviceScope.launch { messageHandler.handleSosDelivered(routed) }
            }
        }
