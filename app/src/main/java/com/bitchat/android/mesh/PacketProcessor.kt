...
class PacketProcessor(private val myPeerID: String) {
    ...
    private suspend fun handleReceivedPacket(routed: RoutedPacket) {
        ...
        when (messageType) {
            ...
            MessageType.SOS_DELIVERED -> handleSosDelivered(routed)
            else -> {
    ...
    private suspend fun handleSosDelivered(routed: RoutedPacket) {
        val peerID = routed.peerID ?: "unknown"
        Log.d(TAG, "Processing SOS_DELIVERED from ${formatPeerForLog(peerID)}")
        delegate?.handleSosDelivered(routed)
    }
...
interface PacketProcessorDelegate {
    ...
    fun handleSosDelivered(routed: RoutedPacket)
