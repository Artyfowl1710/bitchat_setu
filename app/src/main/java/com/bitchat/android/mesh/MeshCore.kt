    ...
    private fun setupDelegates() {
        ...
        packetProcessor.delegate = object : PacketProcessorDelegate {
            ...
            override fun handleSosRelay(routed: RoutedPacket) {
                ...
                val sosPacket = com.bitchat.android.model.SosRelayPacket.decode(packet.payload) ?: return
                
                // Track seen SOS messages to avoid loops (deduplication)
                // Assuming we use a simple set or rely on the fact that if we have internet, we just forward it.
                // We should make a network request if we have internet.
                scope.launch {
                    try {
                        val hasInternet = try {
                        ...
                        if (hasInternet) {
                            // Hit backend using HttpURLConnection
                            val url = java.net.URL("http://10.88.81.73:3000/relay-sos")
                            ...
                            val json = org.json.JSONObject().apply {
                                put("messageId", sosPacket.messageId)
                                put("destinationPhoneNumber", sosPacket.destinationPhoneNumber)
                                put("text", sosPacket.text)
                            }
                            ...
                            if (responseCode in 200..299) {
                                // Create SOS_DELIVERED
                                val deliveredPacket = BitchatPacket(
                                    version = 1u,
                                    type = MessageType.SOS_DELIVERED.value,
                                    senderID = MeshPacketUtils.hexStringToByteArray(myPeerID),
                                    recipientID = SpecialRecipients.BROADCAST,
                                    timestamp = System.currentTimeMillis().toULong(),
                                    payload = sosPacket.messageId.toByteArray(Charsets.UTF_8),
                                    signature = null,
                                    ttl = maxTtl
                                )
                                val signed = signPacketBeforeBroadcast(deliveredPacket)
                                dispatchGlobal(RoutedPacket(signed))
                                try { gossipSyncManager.onPublicPacketSeen(signed) } catch (_: Exception) { }
                            }
                        } else {
                            // We don't have internet, so we must just let it relay further. 
                            // packetRelayManager will handle forwarding because it's not addressed to us.
                        }
                    } catch (e: Exception) {
                        Log.e("MeshCore", "Failed to process SOS_RELAY: ${e.message}")
                    }
                }
            }

            override fun handleSosDelivered(routed: RoutedPacket) {
                val messageId = String(routed.packet.payload, Charsets.UTF_8)
                // Pass it to message handler or UI to show delivered status
                // If this is our SOS, we mark it delivered
                Log.d("MeshCore", "SOS Delivered: $messageId")
                delegate?.didReceiveDeliveryAck(messageId, routed.peerID ?: "unknown")
            }
        }
    }
