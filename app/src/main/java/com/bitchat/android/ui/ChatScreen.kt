...
fun ChatScreen(viewModel: ChatViewModel) {
    ...
    // SOS Status Banner — shown above input row when SOS is active
    SosStatusIndicator(
        state = sosStatus,
        onDismiss = { sosManager.dismissStatus() }
    )

    // SOS Config Sheet — shown on first tap if no contact saved
    if (showSosConfig) {
        SosConfigSheet(
    ...
    Row(
        ...
        // Normal input (takes remaining width)
...
fun ChatInputSection(
    ...
    Surface(
        ...
        Column {
            ...
            // Emergency preset chips — visible when text field is empty
            if (messageText.text.isEmpty()) {
                Row(
                    ...
                    val presets = listOf(
                        androidx.compose.ui.res.stringResource(com.bitchat.android.R.string.preset_water),
                        androidx.compose.ui.res.stringResource(com.bitchat.android.R.string.preset_medical),
                        androidx.compose.ui.res.stringResource(com.bitchat.android.R.string.preset_food)
                    )
                    presets.forEach { preset ->
                        androidx.compose.material3.AssistChip(
                            onClick = {
                                onMessageTextChange(androidx.compose.ui.text.input.TextFieldValue(preset))
                                onSend()
                            },
                            label = {
                                Text(
