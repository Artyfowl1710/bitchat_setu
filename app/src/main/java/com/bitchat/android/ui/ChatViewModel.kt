...
class ChatViewModel(
    ...
    /** Broadcasts a raw BitchatPacket (e.g. SOS_RELAY) through the mesh. */
    fun broadcastRawPacket(packet: com.bitchat.android.protocol.BitchatPacket) {
        viewModelScope.launch(kotlinx.coroutines.Dispatchers.IO) {
            try {
                mesh.sendRawBroadcast(packet)
            } catch (e: Exception) {
                android.util.Log.e(TAG, "Failed to broadcast raw packet: ${e.message}")
            }
        }
    }
