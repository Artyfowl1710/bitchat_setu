...
class MainActivity : OrientationAwareActivity() {
    ...
    private fun OnboardingFlowScreen(modifier: Modifier = Modifier) {
        ...
        when (onboardingState) {
            OnboardingState.WELCOME_INTRO -> {
                com.bitchat.android.onboarding.WelcomeIntroScreen(
                    modifier = modifier,
                    onGetStarted = {
            ...
            OnboardingState.PRE_ENTRY_SOS -> {
                com.bitchat.android.ui.SetuPreEntryScreen(
                    onEnterSetu = {
    ...
    private fun proceedWithPermissionCheck() {
        ...
        lifecycleScope.launch {
            ...
            if (permissionManager.isFirstTimeLaunch()) {
                Log.d("MainActivity", "First time launch, showing welcome intro")
                com.bitchat.android.identity.DisplayNameGenerator.getOrGenerateDisplayName(this@MainActivity)
                mainViewModel.updateOnboardingState(OnboardingState.WELCOME_INTRO)
    ...
    private fun initializeApp() {
        ...
        lifecycleScope.launch {
            try {
                ...
                mainViewModel.updateOnboardingState(OnboardingState.PRE_ENTRY_SOS)
            } catch (e: Exception) {
